...
interface RiffNote extends NoteInfo {
  duration: number;
  delay: number;
}
...
class Accompaniment implements IAccompaniment {
  ...
  /**
   * 播放口琴音色
   * @param note - 音符名称 (如 'C', 'D#')
   * @param octave - 八度
   * @param duration - 持续时间(秒)
   * @param volume - 音量 (0-1)
   */
  playHarmonica(note: string, octave: number = 4, duration: number = 0.5, volume: number = 0.4): void {
   ...
   * @param isMuted - 是否闷音
   */
  playGuitar(note: string, octave: number = 3, duration: number = 1.0, volume: number = 0.5, isMuted: boolean = false): void {
    if (!this.isInitialized || !this.audioContext) return;
   ...
   * @param duration - 持续时间
   * @param volume - 音量
   * @param strumPattern - 扫弦模式
   */
  playGuitarChord(notes: NoteInfo[], duration: number = 1.5, volume: number = 0.4, strumPattern: StrumPattern = 'down'): void {
   ...
   * @param volume - 音量
   */
  playHarmonicaBluesRiff(rootNote: string, volume: number = 0.4): void {
    if (!this.isInitialized) return;
   ...
   * @returns 和弦音符数组
   */
  getChordNotes(rootNote: string): NoteInfo[] {
